...
def parse():
    ...
    cl_parser.add_argument(
        "-a", "--all", action="store_true", help="bootstrap all libraries; overrides --library and --library-file"
    )
    cl_parser.add_argument("-f", "--force", action="store_true", help="force updating, i.e. disregard state file")
    cl_parser.add_argument(
        "-j", "--json-file", default="test/libraries.json", help="specifies JSON file to read library metadata from"
    )
    cl_parser.add_argument(
        "-p",
        "--postprocessing-dir",
    ...
    cl_parser.add_argument(
        "--force-fallback", action="store_true", help="enforces use of the fallback server under the specified URL"
    )
    cl_parser.add_argument(
        "-v",
        "--verbose",
